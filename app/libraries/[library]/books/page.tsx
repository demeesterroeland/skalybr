...
function parseUrlState(searchStr: string) {
  ...
  return { viewMode, filterLayout, filters };
}
...
export default function HomePage() {
  ...
  const currentLibrary = decodeURIComponent((params.library as string) || DEFAULT_LIBRARY_NAME);
  ...
  useEffect(() => {
    ...
    const initialUrl = serializeUrlState(vm, fl, f);
    window.history.replaceState(null, '', initialUrl);
  }, []);

  // Sync state to URL in address bar whenever filters, view, or sort change
  useEffect(() => {
    if (!isInitialized.current) return;
    const newUrl = serializeUrlState(viewMode, filterLayout, filters);
    window.history.replaceState(null, '', newUrl);
  }, [viewMode, filterLayout, filters]);
  ...
  const { data: facets } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/v1/libraries/${encodeURIComponent(currentLibrary)}/facets`);
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) => {
      ...
      const res = await fetch(`/api/v1/libraries/${encodeURIComponent(currentLibrary)}/books?${queryParams.toString()}`);
      const json = await res.json();
  ...
  const handleSelectLibrary = (newLibrary: string) => {
    router.push(`/libraries/${encodeURIComponent(newLibrary)}/books`);
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950">
      {/* Top Navigation */}
      <Header currentLibrary={currentLibrary} />
